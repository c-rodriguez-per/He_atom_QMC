...
def metropolis_sample(pos,wf,tau=0.01,nstep=1000, M = 10, block_length = 100):
  ...
  #s_new = np.broadcast_to(s, (3, pos.shape[2]))
  s_new = np.tile(s[:, np.newaxis], (1, pos.shape[2]))
  ...
  for j in range(Nblocks):
    for istep in range(block_length):
      ...
      #for i in range(len(x1_index)):
        #rho_acc[x1_index[i], y1_index[i], z1_index[i]] += 1
        # Convert the 1D indices arrays to a single 2D array
      ...
      # Add counts to rho_acc at unique indices
      rho_acc[j,unique_indices[:, 0], unique_indices[:, 1], unique_indices[:, 2]] += counts

      #rho_acc[x1_index,y1_index,z1_index] += 1
      ...
      #rho_acc[x2_index,y2_index,z2_index] += 1

  
  print(np.sum(rho_acc))
...
def test_metropolis(
  ...
  possample,acc,df,density = metropolis_sample(possample,wf,tau=tau,nstep=nstep,M=M)

  print(acc)
  #print(density)
  np.save('n_v', density) #to open s = np.load(open('n(r).npy','rb'))
  #print(np.sum(rho))
  import pandas as pd
